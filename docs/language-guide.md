...
`double`: the language has no infinities or NaN. Bitwise `&`, `|`, `^`, `~` and the
shifts `<<`, `>>`, `>>>` instead wrap modulo 2^64 (two's complement): `>>` is
arithmetic (sign-extending), `>>>` is logical (zero-filling), and a shift count
outside `0..63` is a runtime error.
