...
static void emitCpp(const std::filesystem::path& header, const std::filesystem::path& out, const std::string& ns, const std::vector<Fn>& fns, const std::vector<NativeClass>& classes) {
    ...
    for (const auto& f : fns) o << " t.push_back(NativeFunction{NativeId::EXTENSION, \"" << ns << "." << f.name << "\", " << f.name << "_binding});\n";
    for (const auto& c : classes) {
        o << " t.push_back(NativeFunction{NativeId::EXTENSION, \"" << ns << "." << c.name << ".new\", " << c.name << "_new_binding});\n";
        o << " t.push_back(NativeFunction{NativeId::EXTENSION, \"" << ns << "." << c.name << ".close\", " << c.name << "_close_binding});\n";
        if (c.ownership == "shared") o << " t.push_back(NativeFunction{NativeId::EXTENSION, \"" << ns << "." << c.name << ".retain\", " << c.name << "_retain_binding});\n";
        for (const auto& m : c.methods) o << " t.push_back(NativeFunction{NativeId::EXTENSION, \"" << ns << "." << c.name << "." << m.name << "\", " << c.name << "_" << m.name << "_binding});\n";
    }
